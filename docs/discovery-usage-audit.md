...
### i98: distinguish index misses from Lean declaration absence

Telemetry 153111 reported an exact miss for `UniformSpace.toTopologicalSpace`, an inherited projection of `UniformSpace` (which extends `TopologicalSpace`). Exact misses now explicitly say “not found in index.” When the requested parent is an indexed structure/class, a short note explains that generated members may be unindexed and gives the existing importing-file `#inspect` route. The note does not assert that the requested member exists; unknown parents and ambiguous names retain their existing routing.

Validation: 144 search tests pass, including an indexed-parent/missing-projection regression and unknown-parent control. An isolated `Derived extends Base` check succeeds and `#inspect Derived.toBase` returns `Derived → Base`, with no axioms. The CLI fixture's name-only lookup was warming, so steady-state wording/handoff is verified by the indexed regression rather than that replay. Existing candidate caps, verbs and index format are unchanged.

### i99: put compact type differences directly in long check previews

The selected completed instance-mismatch episode (153075–153125, s4017) had eight failed checks and no use of the offered context probe. Diagnostic c54859 contains 1,131 characters, but its differing expressions total 229 characters after removing common prefix/suffix tokens. The existing type-difference extractor is now reused in check previews when the original exceeds half the diagnostic budget and the complete focused preview is less than half its length. Truncated differences, short diagnostics and weak reductions retain the original rendering. The location and rejected term remain; omitted shared context is labeled and full diagnostics are linked through `show`.

Validation: all ten daemon tests pass, covering distinct instances, short/unrelated fallbacks and existing goal/check behavior. `/tmp/mm-type-preview.py` verifies a generic dependent-type mismatch with many shared arguments: the CLI exposes `actual: 0` / `expected: 1`, and `show --all` retains the full types. This does not prove a causal reduction in fleet attempts. No new verbs or changes to stored diagnostics.

### i100: apply character thresholds consistently to Unicode queries
...
### i103: deliver existing applicability evidence during repeated rewrite failures

The workflow-level review found 35 failed checks across two recorded episodes, with global-only probes and no uptake of check-context probes. Controlled positive, incidental-overlap and unmet-premise cases confirmed that useful conversion evidence already existed behind that extra command. Check now reuses that retrieval when the existing repeated-blocker detector fires on a rewrite failure. It presents at most the first candidate, explicitly labeled as an indexed signature with unverified applicability and import availability. The full candidate text must fit 480 characters; otherwise it is omitted. The initial failure, unsupported diagnostics and empty retrieval keep their existing output. Retrieval errors do not replace the check failure. No automatic Lean proof search is added.

Validation: 147 search tests and ten daemon tests pass. Three isolated repeated-check replays verify useful recovery followed by a passing check, suppression of incidental name/documentation overlap, and visible `False` premises with positioned inspection. The regular CLI smoke now includes the positive repeated-failure/repaired-check workflow; the independent release gate will run it. The existing full context command still uses the same retrieval and retains deeper evidence. These fixtures establish behavior, not measured fleet productivity gains.


### i110: recover protected alias source

Telemetry 153685–153692 repeatedly requested signatures and source for `ContinuousLinearMap.sum_apply` and `coe_sum`, receiving artifact-only fallbacks. Their Mathlib source explicitly declares attributed `protected alias` commands. The alias source matcher accepted attributes and ordinary aliases but omitted the protected modifier. It now accepts that modifier while preserving exact namespace matching, comment masking, the original source body and the existing origin handoff. This recovers textual generator evidence; it does not fabricate an elaborated signature.

The existing alias regression failed on the protected form before the change and passes after it, with ordinary, attributed, multiline, namespace and comment controls. A standalone Lean-only fixture could not elaborate Batteries alias syntax and is not counted as an end-to-end verification. No public verbs, index schema, or Lean service behavior change.
