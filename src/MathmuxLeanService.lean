    ...
    let detail ← if request.operation == "goal" then
        ...
        let action : Tactic.TacticM String := do
          ...
          if remaining.isEmpty then
            for original in mvars do
              let status ← original.withContext do
      ...
      | some goal =>
        let some mvar := (goalMVars goal).head?
          | throw <| IO.userError "probe position has no active goal; choose a line inside a proof"
...
partial def collectAfterError
  ...
  let commandMessages := command.diagnostics.msgLog ++ result.diagnostics.msgLog ++ result.cmdState.messages
...
partial def firstErrorOrFinal (task : Language.SnapshotTask Language.Lean.CommandParsedSnapshot)
  ...
  let messages := command.diagnostics.msgLog ++ result.diagnostics.msgLog ++ result.cmdState.messages
  if messages.hasErrors then
    cancelCommandWork command
    if let some next := command.nextCmdSnap? then
