...
pub(super) fn declaration_regex() -> &'static Regex {
    ...
    REGEX.get_or_init(|| {
        Regex::new(
            r"(?m)^[ \t]*(?:@\[[^\n]*\][ \t]*(?:\n[ \t]*)*)*(?:(?:public|private|protected|noncomputable|nonrec|unsafe|partial|scoped|local)[ \t]+)*(?P<kind>theorem|lemma|def|abbrev|opaque|axiom|structure|class|inductive|instance)\s+(?:\([ \t]*priority[ \t]*:=[^\n)]*\)[ \t]+)?(?P<name>[\p{L}_][\p{L}\p{N}\p{M}_'.]*)?",
        )
...
pub(super) fn alias_source_entry(source: &str, requested: &str) -> Option<SourceEntry> {
    ...
    let alias = ALIAS.get_or_init(|| Regex::new(
        r"(?m)^[ \t]*(?:@\[[^\n]*\][ \t]*(?:\n[ \t]*)*)*(?:protected[ \t]+)?alias[ \t]+(?P<names>⟨[^⟩]+⟩|[\p{L}_][\p{L}\p{N}\p{M}_'.]*)[ \t]*:=[ \t\r\n]*(?P<origin>[\p{L}_][\p{L}\p{N}\p{M}_'.]*)"
