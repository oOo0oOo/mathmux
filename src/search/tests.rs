...
#[test]
fn tuning_sql_exposes_fts_weights() {
    let sql = ranked_rows_sql("WHERE search_fts MATCH ?1");
    assert!(sql.contains(&fts_rank_sql()), "{sql}");
    assert!(sql.ends_with("WHERE search_fts MATCH ?1"), "{sql}");
}
...
fn query_parsing_scoring_and_ranking_regressions() {
    ...
    assert!(!should_query_exact_leaf(false, false));
    assert!(should_query_exact_leaf(true, false));
    assert!(should_query_exact_leaf(false, true));
...
#[test]
fn source_parser_does_not_reuse_docs_after_top_level_commands() {
    let source = "/-- Addition documentation. -/\ntheorem addition : True := trivial\n\nsection Mul\ntheorem target : True := trivial\nend Mul\n";
    let entries = parse_source(source, "Demo");
    let target = entries.iter().find(|entry| entry.name == "target").unwrap();
...
fn alias_source_requires_exact_generated_name_and_exposes_origin() {
    ...
    let attributed = "namespace Demo\n@[deprecated (since := \"2026-03-05\")] alias old :=\n  current\nend Demo\n";
    let entry = alias_source_entry(attributed, "Demo.old").unwrap();
    assert_eq!(entry.line, 2);
    assert!(entry.body.starts_with("@[deprecated"));
...
fn theorem_signatures_keep_top_level_let_bindings_and_conclusion() {
    ...
    let theorem = entries
        .iter()
        .find(|entry| entry.name == "Demo.longLetType")
        .unwrap();
    assert!(theorem.signature.contains("let eSource : Type := X"));
    assert!(theorem.signature.contains("letI : Inhabited eSource :="));
    assert!(theorem.signature.contains("∃ y : eSource, y = y"));
    assert!(!theorem.signature.contains("exact ⟨default, rfl⟩"));

    let definition = entries
        .iter()
        .find(|entry| entry.name == "Demo.bodyLet")
        .unwrap();
    assert_eq!(definition.signature, "Nat");
}
