...
pub fn render_formalization_yaml(repo: &Repo, state: &State) -> Result<String> {
    ...
    Ok(publication_yaml(project, license, sorries))
}

fn publication_yaml(project: &str, license: Option<&str>, sorries: Option<usize>) -> String {
...
version: \"v0.4\"\n",
    ...
    if sorries == Some(0) {
        writeln!(output, "  sorry_count: 0").unwrap();
        writeln!(output, "  sorry_in_definitions: 0").unwrap();
    } else if let Some(sorries) = sorries {
        writeln!(
            output,
            "  # TODO: count source occurrences; compiler audit found {sorries} sorry-bearing declaration(s)"
        )
        .unwrap();
    }
...
mod tests {
    ...
    fn publication_yaml_contains_only_upstream_metadata() {
        let yaml = publication_yaml("demo", Some("MIT"), Some(0));
        let top_level = yaml
            .lines()
